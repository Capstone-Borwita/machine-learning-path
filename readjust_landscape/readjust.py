...
model = YOLO("path-to-model")#path model

# Define the class ID you want to detect, e.g., ID field or name field in the KTP
TARGET_CLASS_ID = 0  # Adjust as necessary

def detect_field_in_rotated_image(image_path, target_class_id=TARGET_CLASS_ID, confidence_threshold=0.6):
    """
    ...
    while angle < 360:
        ...
        results = model.predict(source=rotated_image, conf=confidence_threshold, save=False)
    ...
    if best_detection is not None:
        ...
        cv2.rectangle(best_rotated_image, (x1, y1), (x2, y2), color=(0, 255, 0), thickness=2)
        ...
        cv2.rectangle(best_rotated_image, (text_offset_x, text_offset_y - text_height - 2),
                      (text_offset_x + text_width, text_offset_y), (0, 255, 0), -1)

        # Place label text
        cv2.putText(best_rotated_image, label, (text_offset_x, text_offset_y - 2), 
...
# Example usage
image_path = "path-to-image.jpg" #path gambar
detect_field_in_rotated_image(image_path)
