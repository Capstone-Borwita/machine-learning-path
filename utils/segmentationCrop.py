...
def process_single_ktp_image(input_image_path):
    ...
    def is_whole_ktp(mask):
        ...
        mask_binary = (
            cv2.cvtColor(mask_binary, cv2.COLOR_BGR2GRAY)
        ...
        return not any(
            [np.any(top_row), np.any(bottom_row), np.any(left_col), np.any(right_col)]
        )
    ...
    def wrap_ktp(mask, original_image):
        ...
        if four_points is None:
            ...
            scale_x, scale_y = (
                original_image.shape[1] / size[0],
                original_image.shape[0] / size[1],
            )
            return original_image[
                int(y * scale_y) : int((y + h) * scale_y),
    ...
    for result in results:
        if result.masks is not None and len(result.masks.data) > 0:
            ...
            if not is_whole_ktp(mask):
                return (
                    None,
                    "KTP terdeteksi tidak lengkap. Pastikan seluruh bagian KTP terlihat dalam gambar.",
                )

            return (wrap_ktp(mask, image), None)

    return (None, "Gambar gagal diproses")
